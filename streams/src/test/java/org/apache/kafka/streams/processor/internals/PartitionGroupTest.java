...
public class PartitionGroupTest {
    ...
    public void shouldNeverWaitIfIdlingIsDisabled() {
        ...
        try (final LogCaptureContext logCaptureContext = LogCaptureContext.create(
            this.getClass().getName() + "#shouldNeverWaitIfIdlingIsDisabled",
    ...
    public void shouldBeReadyIfAllPartitionsAreBuffered() {
        ...
        try (final LogCaptureContext logCaptureContext = LogCaptureContext.create(
            this.getClass().getName() + "#shouldBeReadyIfAllPartitionsAreBuffered",
            Collections.singletonMap(PartitionGroup.class.getName(), "TRACE"))) {
            assertThat(group.readyToProcess(0L), is(true));
            assertThat(
                logCaptureContext.getMessages(),
    ...
    public void shouldWaitForFetchesWhenMetadataIsIncomplete() {
        ...
        assertThat(group.allPartitionsBufferedLocally(), is(false));
        try (final LogCaptureContext logCaptureContext = LogCaptureContext.create(
            this.getClass().getName() + "#shouldWaitForFetchesWhenMetadataIsIncomplete",
            Collections.singletonMap(PartitionGroup.class.getName(), "TRACE"))) {
    ...
    public void shouldWaitForPollWhenLagIsNonzero() {
        ...
        try (final LogCaptureContext logCaptureContext = LogCaptureContext.create(
            this.getClass().getName() + "#shouldWaitForPollWhenLagIsNonzero",
            Collections.singletonMap(PartitionGroup.class.getName(), "TRACE"))) {
            assertThat(group.readyToProcess(0L), is(false));
            assertThat(
                logCaptureContext.getMessages(),
    ...
    public void shouldIdleAsSpecifiedWhenLagIsZero() {
        ...
        try (final LogCaptureContext logCaptureContext = LogCaptureContext.create(
            this.getClass().getName() + "#shouldIdleAsSpecifiedWhenLagIsZero",
            Collections.singletonMap(PartitionGroup.class.getName(), "TRACE"))) {
            assertThat(group.readyToProcess(0L), is(false));
            assertThat(
                logCaptureContext.getMessages(),
            ...
            assertThat(
                logCaptureContext.getMessages(),
                hasItem(
                    "TRACE [test] Continuing to process although some partitions are empty on the broker.\n" +
                        "\tThere may be out-of-order processing for this task as a result.\n" +
