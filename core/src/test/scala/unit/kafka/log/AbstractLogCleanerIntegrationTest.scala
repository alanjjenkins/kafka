...
import org.apache.kafka.common.record.{CompressionConfig, MemoryRecords, RecordBatch}
...
abstract class AbstractLogCleanerIntegrationTest {
  ...
  def writeDups(numKeys: Int, numDups: Int, log: Log, compressionConfig: CompressionConfig,
    ...
    for(_ <- 0 until numDups; key <- startKey until (startKey + numKeys)) yield {
      ...
      val appendInfo = log.appendAsLeader(TestUtils.singletonRecords(value = value.toString.getBytes, codec = compressionConfig.getType,
