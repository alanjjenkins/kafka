...
object KafkaConfig {
  ...
  val CompressionLevelProp = "compression.level"
  ...
  val CompressionLevelDoc = "The compression level for all data generated by the producer. The default level and valid value is up to " +
    s"compression.type. (<code>none</code>, <code>snappy</code>: not available. <code>gzip</code>: [1,...,9]. <code>lz4</code>: [1,...,17]. " +
  ...
  private[server] val configDef = {
    ...
    new ConfigDef()
      ...
      .define(CompressionLevelProp, STRING, Defaults.CompressionLevel, MEDIUM, CompressionLevelDoc)
...
class KafkaConfig private(doLog: Boolean, val props: java.util.Map[_, _], dynamicConfigOverride: Option[DynamicBrokerConfig])
  ...
  def compressionLevel = getString(KafkaConfig.CompressionLevelProp)
